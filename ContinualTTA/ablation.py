...
# CIFAR_MODEL_PATH  = r"C:\Users\Vineet9.Yadav\Desktop\ContinualTTA\resnet50_cifar10_source.pth"
# CIFAR_DATA_DIR    = r"C:\Users\Vineet9.Yadav\Desktop\ContinualTTA\CIFAR-10-C\CIFAR-10-C"
...
# =============================================================================
# ContinualTTA — CIFAR-10-C Ablation Study (Truly Continual)
#
# Variants:
#   A0  Full ContinualTTA          – JS gate + entropy filter
#   A1  No JS Detector             – always adapt (remove gate)
#   A2  KL Detector                – replace JS with KL divergence
#   A3  No Entropy Filter          – use all samples regardless of entropy
#
# Protocol: Truly continual, S1–S5, no reset between corruptions or severities.
# =============================================================================
...
RESULTS_DIR = r"C:\Users\Vineet9.Yadav\Desktop\ContinualTTA\results\ablation_continual"
os.makedirs(RESULTS_DIR, exist_ok=True)
...
JS_THRESH = 0.04
SAR_RHO   = 0.05     # not used here, kept for completeness
...
# ------------------------- SHIFT DETECTORS -------------------------
def js_divergence(p_ref, p_t):
    m = 0.5 * (p_ref + p_t)
    kl_1 = F.kl_div(m.log().unsqueeze(0), p_ref.unsqueeze(0), reduction="batchmean")
    kl_2 = F.kl_div(m.log().unsqueeze(0), p_t.unsqueeze(0), reduction="batchmean")
    return 0.5 * (kl_1 + kl_2)

def kl_divergence(p_ref, p_t):
    return F.kl_div(p_ref.log().unsqueeze(0), p_t.unsqueeze(0), reduction="batchmean")

# ------------------------- VARIANT BUILDER -------------------------
def make_variant(source, use_detector, detector_type, use_filter):
    ...
    def fn(x):
        ...
        # ---- Gate ----
        adapt = True   # default: always adapt if no detector
        if use_detector:
            with torch.no_grad():
                ...
                if detector_type == "js":
                    div = js_divergence(reference[0], p_t)
                elif detector_type == "kl":
                    div = kl_divergence(reference[0], p_t)
                else:
                    div = 0.0
                reference[0] = 0.9 * reference[0] + 0.1 * p_t
                adapt = div.item() > JS_THRESH
        ...
        if use_filter:
            ...
            reliable = torch.ones(logits.size(0), dtype=torch.bool, device=DEVICE)

        # ---- Update ----
        loss = softmax_entropy(logits[reliable]).mean()
        loss.backward()
...
def run_ablation(variant_name, variant_kwargs, source):
    print(f"\n{'='*60}")
    print(f"Variant: {variant_name}")
    print(f"Detector: {variant_kwargs['use_detector']} ({variant_kwargs['detector_type']}), "
          f"Filter: {variant_kwargs['use_filter']}")
    print(f"{'='*60}")

    fn = make_variant(source, **variant_kwargs)
    ...
    for severity in SEVERITIES:
        ...
        save_severity(variant_name, severity, all_results[severity])

    return all_results

# ------------------------- SAVE -------------------------
def save_severity(variant, severity, results):
    path = os.path.join(RESULTS_DIR, f"{variant}_s{severity}.csv")
    mean = np.mean(list(results.values()))
    with open(path, "w") as f:
        f.write(f"corruption,{variant}_s{severity}\n")
        for c in ALL_CORRUPTIONS:
            f.write(f"{c},{results[c]:.4f}\n")
        f.write(f"Mean,{mean:.4f}\n")

def compute_averaged(variant, all_results):
    averaged = {}
    for c in ALL_CORRUPTIONS:
        averaged[c] = np.mean([all_results[s][c] for s in SEVERITIES])
    mean_overall = np.mean(list(averaged.values()))
    path = os.path.join(RESULTS_DIR, f"{variant}_averaged.csv")
    with open(path, "w") as f:
        f.write(f"corruption,{variant}\n")
        for c in ALL_CORRUPTIONS:
...
if __name__ == "__main__":
    ...
    for vname, vkwargs in VARIANTS.items():
        res = run_ablation(vname, vkwargs, source)
        avg, mean = compute_averaged(vname, res)
        results_summary[vname] = mean
        print(f"\n  {vname} S1-S5 mean: {mean:.2f}%")

    print("\n" + "="*50)
    print("ABLATION SUMMARY")
    print("="*50)
    for vname in VARIANTS:
        print(f"{vname:<20} {results_summary[vname]:.2f}%")